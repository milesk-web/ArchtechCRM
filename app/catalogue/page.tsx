...
export default function CataloguePage() {
  ...
  async function load() {
    ...
    try {
      ...
      setFlashingBands(flashingBandsData);
  ...
  async function loadProfiles(): Promise<Profile[]> {
    return fetchCatalogue<Profile>("profiles", (row) => ({
      id: String(row.id),
  ...
  async function loadFlashings(): Promise<FlashingType[]> {
  ...
  async function loadFlashingBands(): Promise<FlashingGirthBand[]> {
    return fetchCatalogue<FlashingGirthBand>(
      "flashing_girth_bands",
      (row) => ({
        id: String(row.id),
  ...
  async function loadFlashingPrices(): Promise<FlashingPrice[]> {
  ...
  async function loadProfileOptionsFor(
    profileId: string,
  ...
  async function loadMaterialColoursFor(
    materialId: string,
  ...
  async function loadProfileOptions(profileId: string) {
    try {
      setProfileOptions(await loadProfileOptionsFor(profileId));
    } catch (err) {
      setError(
  ...
  async function loadMaterialColours(materialId: string) {
    try {
      setMaterialColours(await loadMaterialColoursFor(materialId));
    } catch (err) {
      setError(
        err instanceof Error
          ? err.message
  ...
  async function loadPricingOptions(profileId: string) {
    try {
      setPricingOptions(await loadProfileOptionsFor(profileId));
    } catch (err) {
      setError(
        err instanceof Error
          ? err.message
  ...
  async function loadPricingColours(materialId: string) {
    try {
      setPricingColours(await loadMaterialColoursFor(materialId));
    } catch (err) {
      setError(
        err instanceof Error
          ? err.message
  ...
  async function addFlashingType() {
    const name = window.prompt("Flashing type name");
  ...
  async function addFlashingBand() {
    ...
    const minGirth = Number(flashingMinGirth);
    ...
    if (
      ...
      setError("Girth values must be numbers greater than or equal to 0.");
      return;
    }

    if (maxGirth < minGirth) {
      setError("Maximum girth must be greater than or equal to minimum girth.");
      return;
    }
    ...
    const created = await createCatalogueItem("flashing_girth_bands", {
      ...
      max_girth: maxGirth,
      active: true,
      sort_order: flashingBands.length,
    });
                  ...
                  <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                    ...
                    <Select
                      ...
                      options={[
                        ...
                        ...pricingOptions.map(
                          (option) =>
                            [option.id, option.value] as [string, string],
                        ),
                      ]}
                    />
                    ...
                    <div className="mt-4 max-w-[280px]">
                      <Select
                        ...
                        options={[
                          ...
                          ...pricingColours.map(
                            (colour) =>
                              [colour.id, colour.name] as [string, string],
                          ),
                        ]}
                      />
  ...
  return (
    <div className="space-y-5">
      <div className="grid gap-5 lg:grid-cols-[1fr_1fr]">
        ...
        >
          {flashings.length === 0 ? (
            <Empty text="No flashing types yet." />
          ) : (
            <div className="divide-y divide-black/[0.06]">
